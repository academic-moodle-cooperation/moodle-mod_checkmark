...
/**
 * JavaScript functionality for checkmark forms, branch 'MOODLE_21_STABLE'
 *
 * @package       mod
...
M.checkmark_local = {
    ...
    init_submission:    function(Y, flexiblenaming, var1, var2, grade) {
            // ensure that data is accessible from all methods of M.checkmark_local
            this.Y = Y;
    ...
    update_summary: function() {
        ...
        var var1 = M.checkmark_local.var1;
        var var2 = M.checkmark_local.var2;
        var grade = M.checkmark_local.grade;
        var flexiblenaming = M.checkmark_local.flexiblenaming;

        if (flexiblenaming) {
            //calculate values using flexible naming (var1 = names[], var2 = grades[])
            for (var i=0; i<var1.length; i++) {
                var number = i+1;
                if (M.checkmark_local.Y.one("input#example".concat(number.toString())) == null) {
                    //compatibility to pre 2.2
                    var is_checked = M.checkmark_local.Y.one("input#id_example".concat(number.toString())).get('checked');
                } else {
                    var is_checked = M.checkmark_local.Y.one("input#example".concat(number.toString())).get('checked');
                }
            ...
            for (i=1; i<=var1; i++) {
                if (M.checkmark_local.Y.one("input#example"+i.toString()+'.checkboxgroup1') == null) {
                    //compatibility to pre 2.2
                    var is_checked = M.checkmark_local.Y.one("input#id_example"+i.toString()+'.checkboxgroup1').get('checked');
                }
                else {
                    var is_checked = M.checkmark_local.Y.one("input#example"+i.toString()+'.checkboxgroup1').get('checked');
                }
                if (is_checked == 1) {
                    count++;
                }
        ...
        M.checkmark_local.Y.one("span#examples").setContent(examples_new.toString());
        M.checkmark_local.Y.one("span#grade").setContent(grade_new.toString());
    ...
    reset_submission_form: function(e) {
        e.preventDefault();
        M.checkmark_local.Y.one("#mform1").reset();
        M.checkmark_local.update_summary();
        return true;
    },

    /*
     * init_settings(Y, dividing_symbol) prepares settings form for JS-functionality
     * first make Y and dividing_symbol accsesible in all other methods of M.checkmark_local
    ...
    init_settings: function(Y, dividing_symbol) {
        ...
        //if (M.checkmark_local.Y.one("#id_flexiblenaming").get('checked')) {
    ...
    update_settings: function() {
        ...
        M.checkmark_local.stripper(null);
        ...
        var examplegrades_selector = '#examplegrades';
        var examplecount_selector = '#examplecount';
        if (M.checkmark_local.Y.one(grade_selector) == null) {
            //compatibility to pre 2.2
            grade_selector = '#id_grade';
            flexiblenaming_selector = '#id_flexiblenaming';
            examplegrades_selector = '#id_examplegrades';
        ...
        if ((M.checkmark_local.Y.one(grade_selector).get('value')==0) || (M.checkmark_local.Y.one(grade_selector).get('value') == -1)) {
            return true;
        }
        if (M.checkmark_local.Y.one(flexiblenaming_selector).get('checked')) {
            ...
            var temp_string = M.checkmark_local.Y.one(examplegrades_selector).get('value').replace(/[^0-9,]/, "");
            var temp_array = temp_string.split(M.checkmark_local.dividing_symbol);
            for (i=0; i<temp_array.length; i++) {
                gradesum += parseInt(temp_array[i]);
            }
        } else {
            //calculate gradesum using example-amount (each example counts 1 point)
            gradesum = M.checkmark_local.Y.one(examplecount_selector).get('value');
        }
        ...
        grade_obj = M.checkmark_local.Y.one(grade_selector);
        //myNode.get('children');
        //myNode.next();
        //myNode.one('> selectorString');

        if (!M.checkmark_local.Y.one(flexiblenaming_selector).get('checked')) {
            if (M.checkmark_local.Y.one(grade_selector).get('value')%gradesum == 0) {
                //grade is integral multiple of gradesum (= examplecount) so everything's fine
                return true;
            }
        }
        if ((gradesum <= 100) && (gradesum >= 0)) {
            if (M.checkmark_local.Y.one(grade_selector.concat(" > [value=\'".concat(gradesum.toString()).concat("\']"))) == null) {
                //alert('M.checkmark_local.Y.one('.concat(grade_selector).concat(" > [value=\'".concat(gradesum.toString()).concat("\']")).concat(') is null'));
                M.checkmark_local.Y.one(grade_selector.concat(" > [value=\'-1\']")).set('selected', true);
            }
            M.checkmark_local.Y.one(grade_selector.concat(" > [value=\'"+gradesum.toString()+"\']")).set('selected', true);
        } else {
            M.checkmark_local.Y.one(grade_selector.concat(" > [value=\'-1\']")).set('selected', true);
        }
        //index = M.checkmark_local.Y.one("#id_grade > [value="+gradesum+"]").get('index');
        //M.checkmark_local.Y.one("#id_grade").set('selectedIndex', index);

        return true;
    },

    stripper: function(e) {
        Y = M.checkmark_local.Y;
