...
class override_actionmenu implements templatable, renderable {
    ...
    protected function get_select_menu(): url_select {
        $userlink = new moodle_url('/mod/checkmark/overrides.php', ['id' => $this->cm->id, 'mode' => 'user']);
        $grouplink = new moodle_url('/mod/checkmark/overrides.php', ['id' => $this->cm->id, 'mode' => 'group']);
        $menu = [
            $userlink->out(false) => get_string('useroverrides', 'mod_checkmark'),
            $grouplink->out(false) => get_string('groupoverrides', 'mod_checkmark'),
        ];
        return new url_select($menu, $this->currenturl->out(false), null, 'mod_checkmark_override_select');
    }
    ...
    public function export_for_template(\renderer_base $output): array {
        ...
        if ($type == 'user') {
            $text = get_string('addnewuseroverride', 'mod_checkmark');
        } else {
            $text = get_string('addnewgroupoverride', 'mod_checkmark');
        }
        $action = ($type == 'user') ? 'adduser' : 'addgroup';

        $params = ['id' => $this->currenturl->get_param('id'), 'action' => $action];
